-- the sidecar client (§7.6): one supervised dipher-sidecar process per nvim
...
---@type dipher.sidecar.Client|nil
local client = nil
...
-- this file is lua/dipher/sidecar/init.lua, so the plugin root is three dirs up.
local function plugin_bin()
    local src = debug.getinfo(1, "S").source:sub(2)
    local root = vim.fn.fnamemodify(src, ":h:h:h:h")
    return root .. "/bin/dipher-sidecar"
end
...
local function resolve_bin()
    ...
    local onpath = vim.fn.exepath("dipher-sidecar")
    if onpath ~= "" then
        return onpath
    end
    return nil
...
function handshake()
    ...
    client.pending[id] = function(err, result)
        if err then
            vim.schedule(function()
                vim.notify(
                    "dipher: sidecar handshake failed: " .. (err.message or err.code),
        ...
        if type(result) ~= "table" or result.protocol ~= PROTOCOL then
            vim.schedule(function()
                vim.notify(
                    "dipher: sidecar protocol mismatch — rebuild your sidecar (run `make go-build` or `go install`)",
                    vim.log.levels.ERROR
                )
            end)
            client.stopping = true
    ...
    send({ id = id, method = "hello", params = { client = "dipher.nvim", protocol = PROTOCOL } })
end
...
function schedule_restart()
    ...
    if client.attempts > MAX_ATTEMPTS then
        vim.schedule(function()
            vim.notify("dipher: sidecar keeps crashing; giving up", vim.log.levels.ERROR)
        end)
...
function start()
    ...
    if not bin then
        return false, "sidecar binary not found (set sidecar_bin or build bin/dipher-sidecar)"
    end
    client.stdout_buf = ""
    client.ready = false
...
-- { protocol, binary }. drives the :Dipher sidecar smoke check.
---@param cb fun(err: table|nil, info: table|nil)
function M.ping(cb)
    M.request("hello", { client = "dipher.nvim", protocol = PROTOCOL }, cb)
end
