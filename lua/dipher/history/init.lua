...
---@field files table<string, dipher.FileEntry[]> -- per-sha lazy file cache (range)
---@field on_select fun(commit: dipher.git.Commit)|nil   -- file mode
---@field expand fun(commit: dipher.git.Commit): dipher.FileEntry[]|nil -- range mode
...
---@field lines string[]
---@field meta (dipher.history.Meta|false)[]
local History = {}
...
---@field expand? fun(commit: dipher.git.Commit): dipher.FileEntry[]
...
---@class dipher.history.CommitCell
---@field sha string
---@field date string
---@field author string
---@field add integer
...
---@param cells dipher.history.CommitCell
...
local function build_commit(cells, w)
    ...
    if cells.refs then
        ...
        local rstart = col
        emit(cells.refs)
        spans[#spans + 1] = { rstart, col, "dipherHistoryRef" }
    end
...
local function build_file(entry)
    ...
    spans[#spans + 1] = { col, col + #entry.status, STATUS_HL[entry.status] or "dipherPanelModify" }
    emit(entry.status)
    emit(" ")
    emit(entry.path)
    emit("  ")
    local add = "+" .. entry.additions
    spans[#spans + 1] = { col, col + #add, "dipherPanelCountAdd" }
    emit(add)
    emit(" ")
    local del = "-" .. entry.deletions
    spans[#spans + 1] = { col, col + #del, "dipherPanelCountDelete" }
    emit(del)
    return table.concat(parts), spans
end
...
function History:render()
    ...
    for ci, cell in ipairs(cells) do
        if self.mode == "range" then
            ...
            shift(spans, #prefix)
            table.insert(spans, 1, { 0, #prefix - 1, "dipherPanelHelp" }) -- the fold arrow
...
---@return dipher.history.Meta|nil
...
function History:show_help()
    local lines = { " dipher file history", "" }
    if self.mode == "range" then
